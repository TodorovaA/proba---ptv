...
while frame_idx < N_FRAMES:
    ...
    def process_mask(mask, color_label):
        ...
        for cnt in contours:
            ...
            local_detections.append((int(x), int(y), color_label, area_px, area_mm2))
    ...
    updated_particles = {}
    used_ids = set()
    for x, y, color, area_px, area_mm2 in detections:
        matched = False
        for pid, (px, py, pcolor, _, _) in active_particles.items():
            if pid in used_ids or color != pcolor:
                continue
            if distance.euclidean((x, y), (px, py)) < MAX_DISTANCE:
                updated_particles[pid] = (x, y, color, area_px, area_mm2)
                used_ids.add(pid)
                matched = True
                break
        if not matched:
            updated_particles[next_particle_id] = (x, y, color, area_px, area_mm2)
            next_particle_id += 1
    ...
    for pid, (x, y, color, area_px, area_mm2) in active_particles.items():
...
df.to_csv("particle_tracking_with_area.csv", index=False)
print("📄 Saved CSV: particle_tracking_with_area.csv")
...
# Tracking setup
next_particle_id = 0
active_particles = {}  # id: (x, y, color, area_px, area_mm2)
trajectories = defaultdict(list)
positions = []
counted_red = set()
counted_blue = set()
...
while frame_idx < N_FRAMES:
    ...
    for pid, (x, y, color, area_px, area_mm2) in active_particles.items():
        ...
        label = f"ID:{pid} A:{area_mm2:.1f}mm²"
        cv2.putText(frame, label, (x + 10, y), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 0), 2)
